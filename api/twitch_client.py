...
class TwitchClient:
    ...
    async def search_categories(self, query):
        ...
        # FIXED: Let requests handle the URL encoding cleanly via the params dict
        ...
        def fetch():
            try:
                ...
                if resp.status_code == 200:
                    ...
                    # --- FIXED: Smart Sorting Algorithm ---
                    # Forces exact matches to the top, then sorts by length so the core game isn't buried by mods
                    data.sort(key=lambda x: (x['name'].lower() != query.lower(), len(x['name'])))
                    
                    return [Category(item['id'], item['name']) for item in data]
            except: pass
            return []
    ...
    async def download_game_art(self, game_name, day_code):
        ...
        def fetch_and_save():
            try:
                # Attempt 1: Standard Twitch Category Art (Properly Encoded)
                url = "https://api.twitch.tv/helix/games"
                resp = requests.get(url, headers=self.headers, params={"name": game_name}, timeout=5)
                box_art_url = None
                
                if resp.status_code == 200:
                    data = resp.json()
                    if data.get('data'):
                        # Scale up to a premium resolution natively
                        box_art_url = data['data'][0]['box_art_url'].replace('{width}', '600').replace('{height}', '800')
                
                # Attempt 2: IGDB API Fallback for Premium High-Res Box Art
                if not box_art_url:
                    igdb_url = "https://api.igdb.com/v4/games"
                    igdb_query = f'search "{game_name}"; fields cover.image_id; limit 1;'
                    igdb_resp = requests.post(igdb_url, headers=self.headers, data=igdb_query, timeout=5)
                    
                    if igdb_resp.status_code == 200:
                        igdb_data = igdb_resp.json()
                        if igdb_data and igdb_data[0].get('cover'):
                            img_id = igdb_data[0]['cover']['image_id']
                            # Pulling the direct high-res 1080p cover asset
                            box_art_url = f"https://images.igdb.com/igdb/image/upload/t_1080p/{img_id}.jpg"

                # Attempt 3: Download the resolved image
                if box_art_url:
