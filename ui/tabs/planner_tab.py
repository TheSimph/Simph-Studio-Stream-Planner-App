...
class PlannerTab(ctk.CTkFrame):
    ...
    def build_weekly_view(self):
        ...
        for i in range(7):
            ...
            # --- Dynamic Responsive Grid Weights ---
            df.grid_columnconfigure(2, weight=4) 
            df.grid_columnconfigure(3, weight=2) 
            df.grid_columnconfigure(4, weight=2) 
            df.grid_columnconfigure((0, 1, 5, 6, 7, 8), weight=0) 
            ...
            g_wrap.grid(row=0, column=2, padx=2, sticky="ew")
            
            gm = ctk.CTkEntry(g_wrap, placeholder_text="Game...", width=50)
            gm.pack(fill="x")
            ...
            sub = ctk.CTkEntry(df, placeholder_text="Sub...", width=50)
            sub.grid(row=0, column=3, padx=2, sticky="ew")
            sub.bind("<KeyRelease>", self.app.schedule_preview)
            
            note = ctk.CTkEntry(df, placeholder_text="Discord Note...", width=50)
            note.grid(row=0, column=4, padx=2, sticky="ew")
            ...
            if saved_layout and len(saved_layout) == 7:
                ...
                if sv.get("check", True): 
                    chk.select()
                else: 
                    chk.deselect()
                    
                game_val = sv.get("game", "")
                if game_val: 
                    gm.insert(0, game_val)
                    
                sub_val = sv.get("sub", "")
                if sub_val: 
                    sub.insert(0, sub_val)
                    
                if "note" in sv:
                    note_val = sv.get("note", "")
                    if note_val: 
                        note.insert(0, note_val)
            ...
            self.days_ui_list.append({
                ...
                "time": tm, 
                "suggest": s_f, 
                "code": self.app.all_days[i], 
                "offline": off_chk, 
                "cancelled": cancel_chk, 
                "custom_art": custom_art_path, 
                "art_btn": art_btn
            })
    ...
    def load_saved_layout_data(self):
        saved_list = self.cfg.get("saved_schedule_layout", [])
        if not saved_list or len(saved_list) != len(self.days_ui_list):
            return
            
        for i, item in enumerate(self.days_ui_list):
            try:
                data = saved_list[i]
                item["check"].set(data.get("check", True))
                
                item["game"].delete(0, 'end')
                game_val = data.get("game", "")
                if game_val: 
                    item["game"].insert(0, game_val)
                
                item["sub"].delete(0, 'end')
                sub_val = data.get("sub", "")
                if sub_val: 
                    item["sub"].insert(0, sub_val)
                
                if "note" in item and "note" in data:
                    item["note"].delete(0, 'end')
                    note_val = data.get("note", "")
                    if note_val: 
                        item["note"].insert(0, note_val)
                    
                item["time"].set(data.get("time", "20:00"))
                item["offline"].set(data.get("offline", False))
                item["cancelled"].set(data.get("cancelled", False))
                item["custom_art"] = data.get("custom_art", None)
            except Exception:
                pass