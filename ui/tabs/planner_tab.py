...
class PlannerTab(ctk.CTkFrame):
    ...
    def build_weekly_view(self):
        self.weekly_frame = ctk.CTkScrollableFrame(self.left_container, width=750, label_text="WEEKLY SCHEDULE TICKBOXES")
        ...
        for i in range(7):
            ...
            ctk.CTkLabel(df, text=self.app.all_days[i], width=50, font=("Arial", 12, "bold")).grid(row=0, column=1)
            
            g_wrap = ctk.CTkFrame(df, fg_color="transparent")
            g_wrap.grid(row=0, column=2, padx=5)
            
            gm = ctk.CTkEntry(g_wrap, width=200, placeholder_text="Game...")
            gm.pack()
            ...
            sub = ctk.CTkEntry(df, placeholder_text="Sub...", width=140)
            sub.grid(row=0, column=3, padx=5)
            ...
            self.days_ui_list.append({
                "check": chk, "game": gm, "sub": sub, "time": tm, 
    ...
    def build_canvas_view(self):
        self.design_frame = ctk.CTkScrollableFrame(self.left_container, width=750, label_text="VISUAL DESIGN")
        ...
        # New 3x2 grid with independent Subtitle and Time text pickers
    ...
    def build_export_view(self):
        self.export_frame = ctk.CTkScrollableFrame(self.left_container, width=750, label_text="EXPORT OPTIONS")
    ...
    def pick_color_generic(self, key):
        ...
        if c[1]: 
            ...
            self.app.settings.save_settings() # Save immediately
            self.app.schedule_preview()
