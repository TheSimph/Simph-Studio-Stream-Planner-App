...
class SimphStudioWindow(ctk.CTk):
    def __init__(self):
        ...
        def cleanup_old_files():
            ...
            try:
                ...
                # --- PREMIUM CACHE GARBAGE COLLECTION (BOOT) ---
    ...
    def on_closing(self):
        ...
        # --- PREMIUM CACHE GARBAGE COLLECTION (SHUTDOWN) ---
        try:
            cache_dir = os.path.join(self.settings.appdata_dir, "art_cache")
            if os.path.exists(cache_dir): shutil.rmtree(cache_dir, ignore_errors=True)
        except: pass
    ...
    async def up_art(self, game_name, day_code):
        ...
        if cache_path and os.path.exists(cache_path):
            ...
            # Move the downloaded file into the hidden AppData cache
            cache_dir = os.path.join(self.settings.appdata_dir, "art_cache")
            os.makedirs(cache_dir, exist_ok=True)
            
            # Delete any old art for this specific day to prevent memory bloat during a session
