...
class SimphStudioWindow(ctk.CTk):
    def __init__(self):
        ...
        self.APP_VERSION = "0.1.66"
        self.REPO_NAME = "Simph-Studio-Stream-Planner-App"
        self.UPDATE_URL = f"https://rawusercontent.com/TheSimph/{self.REPO_NAME}/main/version.txt"
    ...
    def setup_layout(self):
        ...
        self.btn_connections.grid(row=4, column=0, padx=10, pady=5, sticky="ew")
        ...
        self.btn_guide.grid(row=6, column=0, padx=10, pady=(10, 20), sticky="ew")
    ...
    def _download_and_apply_update(self):
        try:
            ...
            if getattr(sys, 'frozen', False):
                ...
                new_exe_path = next((os.path.join(r, d_f, f) for r, d, f_list in os.walk(extract_dir) for f in f_list if f == exe_name), None)
                bat_path = os.path.join(self.settings.appdata_dir, "update.bat")
    ...
    def start_export(self):
        ...
        checked_days = [ {"code": i["code"], "game": i["game"].get(), "sub": i["sub"].get(), "time": i["time"].get(), "offline": i["offline"].get(), "cancelled": i["cancelled"].get(), "custom_art": i["custom_art"]} for i in self.planner_tab.days_ui_list if i["check"].get() ]
    ...
    async def run_engine(self):
        ...
        self.log(f"Discord: {del_msg}")

        sub_text = self.planner_tab.header_sub_entry.get().strip()
        # USE DISCORD H1 MARKDOWN HEADER (#) FOR MAXIMUM SIZE
        ...
        for i, code in enumerate(self.all_days):
            ...
            if item["check"].get():
                ...
                # GRAB THE SUBTITLE AND FORMAT IT
        ...
        up_success, new_msg_id, up_msg = self.discord_client.deploy_schedule(discord_msg, "schedule_deploy.jpg")
        if up_success:
            self.cfg["last_msg_id"] = new_msg_id
            self.settings.save_settings()
            self.log(f"Discord: {up_msg}")
        else:
            self.log(f"Discord: {up_msg}")
