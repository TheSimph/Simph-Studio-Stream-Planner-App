...
class SimphStudioWindow(ctk.CTk):
    def __init__(self):
        ...
        # --- SMART MONDAY START DATE LOGIC ---
        today = datetime.date.today()
        if today.weekday() == 6: 
            # If today is Sunday (6), automatically push forward to tomorrow (Next Monday)
            self.selected_start_date = today + datetime.timedelta(days=1)
        else: 
            # If today is Mon-Sat, snap backward to the current week's Monday
            self.selected_start_date = today - datetime.timedelta(days=today.weekday())
